...
def load_data():
    """
    Load the raw transaction feed and merge it with the
    reconciliation ground truth (scenario label + expected
    settlement) so every downstream agent has a single,
    self-contained DataFrame to work with.

    ``amount`` is added as an alias of ``payment_amount`` so
    agents can reason about the original payment size without
    caring about the raw column naming.
    """
